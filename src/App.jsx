...
function App() {
          ...
          setBufferElapsedSeconds((prev) => {
            ...
            if (currentMins !== lastLoggedMinute) {
              ...
              fetch('https://volttrack-server.onrender.com/api/audit/fine', {
                ...
                body: JSON.stringify({ name: currentUser?.name, evNo: currentUser?.evNo, minutes: currentMins, fineAmount: computedFine })
              }).catch(err => console.error(err));
            }

            if (nextSecs >= 900) {
              alert("Telemetry Timeout: 15-Minute absolute buffer window completely exhausted. Booking dropped.");
  ...
  const handleManualVoidFreeze = () => {
    ...
    alert("Freeze Advantage Manual Bypass: Locked pipeline released.");
  };
  ...
  const executeBooking = (station) => {
    if (!station) return;
    if (!isLoggedIn) {
      alert("Registration profile node missing. Please Sign In or Register at the top right.");
      return;
    }
    if (activeBooking) {
    ...
    const isRebookMode = freezeActive && String(freezeStationId) === String(station.id);

    fetch(`https://volttrack-server.onrender.com/api/stations/${station.id}/book`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: currentUser, isRebook: isRebookMode })
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
  ...
  const handleAdaptiveCancellation = () => {
    ...
    setCancelCount(nextCancelCount);

    if (!isBufferPhase) {
      fetch(`https://volttrack-server.onrender.com/api/stations/${stationId}/cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user: currentUser, type: 'CORE_CANCEL', cancelCount: nextCancelCount })
      })
        .then(res => res.json())
        .then(data => {
          if (data.success) {
      ...
      alert("Core Window Cancellation Confirmed: 5-Minute Freeze advantage enabled.");
    } else {
      fetch(`https://volttrack-server.onrender.com/api/stations/${stationId}/cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert(`Buffer Penalty Settlement Mandate: Settle accrued fine of ₹${accruedFine}.`);
    }
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              ...
              <tbody>
                {filteredStations.map(station => {
                  ...
                  const isCurrentBookingTarget = activeBooking && String(activeBooking.stationId) === String(station.id);
                  ...
                  const statusColor = isFull ? '#ef4444' : '#00e676';
                  ...
                  return (
                    <tr key={station.id} style={{ borderBottom: '1px solid #1a1b20', backgroundColor: isFrozenForMe ? 'rgba(0,230,118,0.03)' : 'transparent' }}>
                      ...
                      <td style={{ padding: '16px 12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.9rem', fontWeight: '500', color: statusColor }}>
                          ...
                          {isFull ? (
                            ...
                            <span>{availableSlots} of {station.totalSlots} Slots Free</span>
                          )}
                        </div>
                      </td>
                      <td style={{ padding: '16px 12px', textAlign: 'right' }}>
                        ...
                        >
                          {isFrozenForMe ? 'Rebook' : isFull ? 'Locked' : 'Inspect'}
                        </button>
                      </td>
                    </tr>
                  );
